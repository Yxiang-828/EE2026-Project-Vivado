...
module oled_keypad (
    ...
    localparam PAGE3_ROWS = 1;
    localparam PAGE3_COLS = 1;
    localparam PAGE3_CELL_WIDTH = 32;
    localparam PAGE3_CELL_HEIGHT = 17;
    localparam PAGE3_X_START = 0;
    ...
    always @(*) begin
        ...
        if (oled_y >= INPUT_Y_START && oled_y <= INPUT_Y_END) begin
            ...
            if (current_page == PAGE_NUMBERS) begin
                ...
                if (oled_x < (PAGE3_COLS * PAGE3_CELL_WIDTH)) begin
                    cell_row_lut = 0;  // only 1 row
                    cell_col_lut = 0;  // only 1 col

                    if (cell_row_lut < PAGE3_ROWS && cell_col_lut < PAGE3_COLS) begin
                        pixel_cell_row = cell_row_lut;
                        pixel_cell_col = cell_col_lut;
                        keypad_cell_row = cell_row_lut;
                        keypad_cell_col = cell_col_lut;

                        char_x_base = PAGE3_X_START + cell_col_lut * PAGE3_CELL_WIDTH + 12;
                        char_y_base = KEYPAD_Y_START + cell_row_lut * PAGE3_CELL_HEIGHT + 4;
                        font_x_offset = oled_x - char_x_base;
                        font_y_offset = oled_y - char_y_base;

                        if (font_y_offset >= 0 && font_y_offset < 8 && font_x_offset >= 0 && font_x_offset < 8) begin
                            cell_font_col = font_x_offset;
                            cell_font_row = font_y_offset;
                            in_keypad_cell = 1;
                        end
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (s1_in_input_area) begin
                s2_char_code <= shared_buffer_char;
