...
module oled_keypad (
    ...
    localparam PAGE_NUMBERS = 2'b00;
    localparam PAGE_FUNCTIONS = 2'b01;
    localparam PAGE_VARIABLES = 2'b10;
    ...
    reg [1:0] current_page = PAGE_NUMBERS;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (enable && btn_pressed[1]) begin  // Up
                ...
                end else if (current_page == PAGE_FUNCTIONS) begin
                    if (selected_row < 2) selected_row <= selected_row + 1;
                end else begin  // PAGE_VARIABLES
                    // Only one row, do nothing
                end
                ...
                if (selected_col == 0 && current_page == PAGE_FUNCTIONS) begin
                    ...
                    selected_row <= 0;
                    selected_col <= 4;
                end else if (selected_col == 0 && current_page == PAGE_VARIABLES) begin
                ...
                if (current_page == PAGE_NUMBERS) begin
                    ...
                    if (selected_col == 2) begin
                        current_page <= PAGE_VARIABLES;
                        selected_row <= 0;
                        selected_col <= 0;
                    end else begin
                        selected_col <= selected_col + 1;
                    ...
                    // Only one column, do nothing
                end
            end else if (enable && btn_pressed[0] && (!vga_buffer_full || ((current_page == PAGE_NUMBERS) && ((selected_row == 0 && selected_col == 4) || (selected_row == 1 && selected_col == 4))))) begin  // Centre - allow clear/delete even when buffer full
                key_valid <= 1;
                ...
                if (current_page == PAGE_NUMBERS) begin
                    ...
                    endcase
                end else if (current_page == PAGE_FUNCTIONS) begin  // PAGE_FUNCTIONS
                    ...
                    if (selected_row == 0 && selected_col == 0) key_code <= KEY_X;
                    else key_code <= 5'b11111;
                end

                // LED control: Light LEDs 0 to 14 based on number key pressed (1-9), equal lights all 15
                case (key_code)
                    KEY_1: leds <= 15'b000000000000001;
                    KEY_2: leds <= 15'b000000000000011;
                    KEY_3: leds <= 15'b000000000000111;
                    KEY_4: leds <= 15'b000000000001111;
                    KEY_5: leds <= 15'b000000000011111;
                    KEY_6: leds <= 15'b000000000111111;
                    KEY_7: leds <= 15'b000000001111111;
                    KEY_8: leds <= 15'b000000011111111;
                    KEY_9: leds <= 15'b000000111111111;
                    KEY_EQUAL: leds <= 15'b111111111111111;
                    default: leds <= 15'b000000000000000;
                endcase
            end
        end
    end
    ...
    always @(*) begin
        ...
        if (oled_y >= INPUT_Y_START && oled_y <= INPUT_Y_END) begin
            ...
            if (current_page == PAGE_NUMBERS) begin
                ...
                // Single cell for X
                if (oled_x >= 0 && oled_x < 96 && oled_y >= KEYPAD_Y_START && oled_y < KEYPAD_Y_START + 52) begin
                    keypad_cell_row = 0;
                    keypad_cell_col = 0;
                    char_x_base = 44;  // Center X
                    char_y_base = KEYPAD_Y_START + 22;  // Center Y
                    font_x_offset = oled_x - char_x_base;
                    font_y_offset = oled_y - char_y_base;
    ...
    always @(*) begin
        ...
        if (s1_in_keypad_cell) begin
            ...
            if (s1_current_page == PAGE_NUMBERS) begin
            ...
            end else if (s1_current_page == PAGE_FUNCTIONS) begin
                ...
                if (s1_keypad_cell_row == 0 && s1_keypad_cell_col == 0) begin
                    selected_char = 8'h58;  // 'X'
                end
            end
