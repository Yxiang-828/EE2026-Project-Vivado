...
module oled_keypad (
    ...
    // CONSTANTS
    localparam KEY_PI = 5'd20;      // 'π'
    localparam KEY_E  = 5'd21;      // 'e'

    // UTILITY
    localparam KEY_DOT   = 5'd22;   // '.'
    localparam KEY_EQUAL = 5'd23;   // '='
    localparam KEY_CLEAR = 5'd24;   // 'C'

    // PARENTHESES
    localparam KEY_LPAREN = 5'd25;  // '('
    localparam KEY_RPAREN = 5'd26;  // ')'

    // CONTROL
    localparam KEY_DELETE = 5'd27;  // 'D' (delete)
    localparam KEY_FACTORIAL = 5'd28; // '!' (factorial)
            ...
            if (enable && btn_pressed[1]) begin  // Up
                ...
                if (current_page == PAGE_NUMBERS) begin
                    case ({selected_row[2:0], selected_col[2:0]})
                        ...
                        default: begin
                            ...
                            if (expression_length < 31) begin
                                case ({selected_row[2:0], selected_col[2:0]})
                                    6'b000_000: begin key_code <= 5'd7; expression_buffer[expression_length] <= 8'h37; end  // '7'
                                    6'b000_001: begin key_code <= 5'd8; expression_buffer[expression_length] <= 8'h38; end  // '8'
                                    6'b000_010: begin key_code <= 5'd9; expression_buffer[expression_length] <= 8'h39; end  // '9'
                                    6'b001_000: begin key_code <= 5'd4; expression_buffer[expression_length] <= 8'h34; end  // '4'
                                    6'b001_001: begin key_code <= 5'd5; expression_buffer[expression_length] <= 8'h35; end  // '5'
                                    6'b001_010: begin key_code <= 5'd6; expression_buffer[expression_length] <= 8'h36; end  // '6'
                                    6'b010_000: begin key_code <= 5'd1; expression_buffer[expression_length] <= 8'h31; end  // '1'
                                    6'b010_001: begin key_code <= 5'd2; expression_buffer[expression_length] <= 8'h32; end  // '2'
                                    6'b010_010: begin key_code <= 5'd3; expression_buffer[expression_length] <= 8'h33; end  // '3'
                                    6'b011_000: begin key_code <= 5'd0; expression_buffer[expression_length] <= 8'h30; end  // '0'
                                    6'b011_001: begin key_code <= 5'd22; expression_buffer[expression_length] <= 8'h2E; end  // '.'
                                    6'b011_010: begin key_code <= 5'd14; expression_buffer[expression_length] <= 8'h5E; end  // '^'
                                    6'b011_011: begin key_code <= 5'd19; expression_buffer[expression_length] <= CHAR_SQRT; end  // '√'
                                endcase
                                expression_length <= expression_length + 1;
                                cursor_pos <= cursor_pos + 1;
                            end
                    ...
                    case ({selected_row[1:0], selected_col[1:0]})
                        ...
                        4'b01_10: begin  // ! (factorial)
                            key_code <= KEY_FACTORIAL;
                            if (expression_length < 31) begin
    ...
    always @(*) begin
        ...
        // Calculate scrolling start index for OLED display (show last 10 characters)
        start_idx = (expression_length > 10) ? expression_length - 10 : 0;

        // ========================================================================
        // INPUT BOX AREA - ANTI-GHOSTING FIX with SCROLLING SUPPORT
        // Shows last 10 characters, scrolling when expression_length > 10
        // ========================================================================
        if (oled_y >= INPUT_Y_START && oled_y <= INPUT_Y_END) begin
            ...
            if (input_font_row < 8) begin  // Valid font row only
                ...
                // Character 9: X range [83, 90]
                    ...
                    if (input_font_col < 8) begin
                        input_char_idx = start_idx + 9;
                        if (input_char_idx < expression_length) in_input_area = 1;
                    end
                end

                // Cursor rendering - Scrolling aware
                if (cursor_visible && cursor_pos >= start_idx) begin
                    cursor_screen_pos = cursor_pos - start_idx;
                    if (oled_x == (2 + cursor_screen_pos * 9)) begin
                        in_cursor_pos = 1;
                    end
                end
            end
